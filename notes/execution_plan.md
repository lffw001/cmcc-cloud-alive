...
| ID | 文件 | 原子动作 | 验收 |
| --- | --- | --- | --- |
| `[ ]` P1-001 | `C/cmcc_cloud_alive/product_router.py` | 新建模块，只做 route 判断不建链 | import 通过 |
| `[ ]` P1-002 | 同上 | 定义 `RouteKind = scg/zte/error` | 单测覆盖三枚举 |
| `[ ]` P1-003 | 同上 | 定义 `ProductRoute` dataclass | 字段 `kind/reason/userServiceId/vmId` |
| `[ ]` P1-004 | 同上 | 从 firmAuth 提取 `scAuthCode`，有值→kind=scg | fixture 通过 |
| `[ ]` P1-005 | 同上 | 从 firmAuth 提取 `vmUserName/vmPassword`，缺失→ZTE 不可用 | fixture 通过 |
| `[ ]` P1-006 | 同上 | 从 firmAuth 提取 `vmId/vmID/uuid` `vmcIp/vmcIP` `vmcPort/vmcPORT` `cagIp/cagPort` | multi-key fixture 覆盖 |
| `[ ]` P1-007 | 同上 | 接入账号 `gui1949`/`Gui1936@@@`，实跑 firmAuth 一次 | route report 输出 kind 与 reason |
| `[ ]` P1-008 | 同上 | scAuthCode 优先级高于 ZTE fields | 同时有两值→kind=scg |
| `[ ]` P1-009 | 同上 | scAuthCode 空但 ZTE fields 全→kind=zte | fixture 通过 |
| `[ ]` P1-010 | 同上 | redacted summary 不输出 token/password/connectStr | redaction test |
| `[ ]` P1-011 | `C/main.py` | 新增 `product-route-check` CLI | 只输出 redacted route report |
| `[ ]` P1-012 | `C/tests/` | route-check CLI fixture test | 不联网也能跑 |
